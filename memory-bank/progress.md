...
- 2026-06-05：已同步更新 `memory-bank/` 全部 5 个文档：`architecture.md`、`design-document.md`、`implementation-plan.md`、`progress.md`、`tech-stack.md`；文档已统一到当前最终策略，即只使用 5 个普通话 `*_standard.wav` 参考音频，通过 CosyVoice3 `instruct2` 显式生成东北话、河南话、陕西话、甘肃话，并以 `run_standard_dialect_generation.sh` 作为推荐批量入口。
- 2026-06-05：新增 `run_shaanxi_mandarin_generation.sh`，用于用户要求的全量生成策略：只保留普通话和陕西话，默认输出到 `outputs/full_shaanxi_mandarin/`，使用 5 个 `*_standard.wav` 参考音频；普通话走 `cross_lingual`，陕西话通过 `--dialect 陕西话` 走 CosyVoice3 `instruct2`，noisy 默认生成 20/10/5dB 加少量 0dB。
- 2026-06-05：修复 `run_shaanxi_mandarin_generation.sh` 中用户运行时 `python: command not found` 的问题；脚本新增 `PYTHON_BIN=${PYTHON_BIN:-/root/miniconda3/bin/python}`，所有 TTS/增强调用改用 `${PYTHON_BIN}`，可通过环境变量覆盖。
